...
function part1(data: Data) {
  ...
  for (let i = 0; i < data.length; i++) {
    const row = data[i];
    const previousRow = i === 0 ? "" : data[i - 1];
    const nextRow = i === data.length - 1 ? "" : data[i + 1];
    ...
    for (let j = 0; j < row.length; j++) {
      ...
      // currently in a number sequence
      if (+curChar >= 0) {
        if (num === "") {
      ...
      // not a number or end of line
      if (j === row.length - 1 || !(+curChar >= 0)) {
        // number was completed
        if (+num > 0) {
          const previousIsSymbol = start > 0 && row.charAt(start - 1) !== ".";
          const nextIsSymbol = !(+curChar >= 0) && curChar !== ".";
          const previousRowSection = previousRow.substring(
            Math.max(start - 1, 0),
            Math.min(j + 1, row.length),
          ).split(
            "",
          );
          const previousRowHasSymbol = previousRowSection.some((e) =>
            !(+e >= 0) && e !== "."
          );
          const nextRowSection = nextRow.substring(
            Math.max(start - 1, 0),
            Math.min(j + 1, row.length),
          ).split("");
          const nextRowHasSymbol = nextRowSection.some((e) =>
            !(+e >= 0) && e !== "."
          );
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = 0; j < row.length; j++) {
      ...
      if (j === row.length - 1 || !(+curChar >= 0)) {
        // number was completed
        if (+num > 0) {
          const previousIsGear = start > 0 && row.charAt(start - 1) == "*";
          if (previousIsGear) {
            if (!gears[`${start - 1}:${i}`]) {
              gears[`${start - 1}:${i}`] = [];
            }
            gears[`${start - 1}:${i}`].push(+num);
          }
          const nextIsGear = !(+curChar >= 0) && curChar == "*";
          if (nextIsGear) {
            if (!gears[`${j}:${i}`]) {
              gears[`${j}:${i}`] = [];
            }
          ...
          previousRowSection.forEach((e, index) => {
            if (e == "*") {
              if (!gears[`${Math.max(start - 1, 0) + index}:${i - 1}`]) {
                gears[`${Math.max(start - 1, 0) + index}:${i - 1}`] = [];
              }
              gears[`${Math.max(start - 1, 0) + index}:${i - 1}`].push(+num);
            }
          });
          const nextRowSection = nextRow.substring(
            Math.max(start - 1, 0),
          ...
          nextRowSection.forEach((e, index) => {
            if (e == "*") {
              if (!gears[`${Math.max(start - 1, 0) + index}:${i + 1}`]) {
                gears[`${Math.max(start - 1, 0) + index}:${i + 1}`] = [];
              }
