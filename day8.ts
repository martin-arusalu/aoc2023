...
function format(
  ...
  const [instructions, _empty, ...nodeStrings] = input.split("\n");
  const nodes: { [label: string]: Node } = {};
  nodeStrings.forEach((n) => {
    const [label, rest] = n.split(" = ");
    const [left, right] = rest.split(", ");
    const L = left.substring(1);
    const R = right.substring(0, 3);
...
function part1(input: string) {
  ...
  let currentNode: Node = nodes.AAA;
  for (let i = 0; i < Number.MAX_SAFE_INTEGER; i++) {
    const ins: "L" | "R" = instructions.charAt(i % instructions.length) as
      | "L"
      | "R";

    if (currentNode[ins] === "ZZZ") {
      result = i + 1;
      break;
    }
    currentNode = nodes[currentNode[ins]];
  }
...
function part2(input: string) {
  ...
  for (const key in nodes) {
    if (key.charAt(2) === "A") startingNodeKeys.push(key);
  }

  function getZPos(nodeKey: string) {
  ...
  for (const nodeKey of startingNodeKeys) {
    const zPos = getZPos(nodeKey);
    if (result === 0) result = zPos;
    else result = utils.lcm(result, zPos);
  }
