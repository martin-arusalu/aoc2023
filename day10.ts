...
function part1(input: string) {
  ...
  function getStartingPoint(): [number, number] {
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < data[i].length; j++) {
        if (data[i].charAt(j) === "S") return [i, j];
      }
    }
  ...
  function getNext(
    cur: [number, number, string[]],
  ): [number, number, string[]] {
    const [row, column, options] = cur;
    const toEast = ["-", "J", "7", "S"];
    const toWest = ["-", "L", "F", "S"];
    const toNorth = ["|", "F", "7", "S"];
    const toSouth = ["|", "L", "J", "S"];
    if (
      ...
      toEast.includes(data[row].charAt(column + 1)) && options.includes("e")
    ) {
      const char = data[row].charAt(column + 1);
      let options = ["e"];
      if (char === "J") options = ["n"];
      if (char === "7") options = ["s"];
      return [row, column + 1, options];
    }
    if (
      row < data.length - 1 && toSouth.includes(data[row + 1].charAt(column)) &&
      options.includes("s")
    ) {
      const char = data[row + 1].charAt(column);
      let options = ["s"];
      if (char === "L") options = ["e"];
      if (char === "J") options = ["w"];
      return [row + 1, column, options];
    }
    if (
      column > 0 && toWest.includes(data[row].charAt(column - 1)) &&
      options.includes("w")
    ) {
      const char = data[row].charAt(column - 1);
      let options = ["w"];
    ...
    if (
      row > 0 && toNorth.includes(data[row - 1].charAt(column)) &&
      options.includes("n")
    ) {
      const char = data[row - 1].charAt(column);
      let options = ["n"];
  ...
  let next = getNext([...start, ["n", "w", "s", "e"]]);
  let steps = 1;
...
function part2(input: string) {
  ...
  for (const point of points) {
    const [row, column, [option]] = point;
    const char = data[row].charAt(column);

    if (option === "e" && char === "-") {
      if (row > 0) side1.push([row - 1, column]);
      if (row < data.length - 1) side2.push([row + 1, column]);
    }
    if (option === "w" && char === "-") {
      if (row > 0) side2.push([row - 1, column]);
      if (row < data.length - 1) side1.push([row + 1, column]);
    }

    if (option === "s" && char === "|") {
      if (column > 0) side2.push([row, column - 1]);
      if (column < data[row].length - 1) side1.push([row, column + 1]);
    }
    if (option === "n" && char === "|") {
      if (column > 0) side1.push([row, column - 1]);
      if (column < data[row].length - 1) side2.push([row, column + 1]);
    }

    if (option === "n" && char === "L") {
      if (column > 0) side1.push([row, column - 1]);
      if (row < data.length - 1) side1.push([row + 1, column]);
      if (row < data.length - 1 && column > 0) {
        side1.push([row + 1, column - 1]);
      }
    }
    if (option === "e" && char === "L") {
      if (column > 0) side2.push([row, column - 1]);
    ...
    if (option === "s" && char === "7") {
      if (column < data[row].length - 1) side1.push([row, column + 1]);
      if (row > 0) side1.push([row - 1, column]);
      if (row > 0 && column < data[row].length - 1) {
        side1.push([row - 1, column + 1]);
      }
    }
    if (option === "w" && char === "7") {
  ...
  for (let i = 0; i < data.length; i++) {
    const line = [];
    for (let j = 0; j < data[i].length; j++) {
      if (points.map((e) => `${e[0]};${e[1]}`).includes(`${i};${j}`)) {
        line.push("X");
      } else if (side1.map((e) => `${e[0]};${e[1]}`).includes(`${i};${j}`)) {
        line.push("O");
      } else if (side2.map((e) => `${e[0]};${e[1]}`).includes(`${i};${j}`)) {
        line.push("I");
        result++;
      } else line.push(data[i].charAt(j));
    }
    newLines.push(line.join(""));
    console.log(line.join(""));
  }

  function getClosest(line: string, j: number) {
    for (let x = 1; x < line.length; x++) {
      if ((j + x) < line.length && ["I", "O"].includes(line.charAt(j + x))) {
        return line.charAt(j + x);
      } else if (j - x >= 0 && ["I", "O"].includes(line.charAt(j - x))) {
        return line.charAt(j - x);
      }
    }
  }
  ...
  for (let i = 0; i < newLines.length; i++) {
    ...
    for (let j = 0; j < line.length; j++) {
      ...
      if (char !== "X" && char !== "I" && char !== "O") {
        const closest = getClosest(line, j);
        if (closest === "I") result++;
      }
    }
  }
